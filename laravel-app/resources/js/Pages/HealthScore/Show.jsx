...
export default function HealthScoreShow({ template, survey, answersByQuestion, history }) {
    ...
    const saveDraft = () => {
        post(route('health-score.surveys.store', { survey: survey.id }, false), {
            preserveScroll: true,
            data: buildPayload(),
        });
    };
    ...
    const submit = () => {
        post(route('health-score.surveys.submit', { survey: survey.id }, false), {
            preserveScroll: true,
            data: buildPayload(),
        });
    };

    const shareUrl = survey?.public_url ?? '';
    ...
    useEffect(() => {
        ...
        const overallRating = ratingFromPercent(overallPct) ?? 0;
        const overallGaugePct = (overallRating / 5) * 100;
        ...
        create('#hs-overall', {
            ...
            plotOptions: {
                radialBar: {
                    ...
                    dataLabels: {
                        name: { offsetY: -8 },
                        value: { formatter: () => `${overallRating.toFixed(1)}/5` },
                    },
                },
            },
            colors: [overallRating >= 4 ? '#22C55E' : overallRating >= 3.5 ? '#F59E0B' : '#EF4444'],
        });
                ...
                <div className="col-lg-4 text-lg-end mt-2 mt-lg-0">
                    ...
                    {overallRating != null ? (
                        <span className="badge badge-primary">
                            {fmt1(overallRating)}/5 {stars(overallRating)}
                        </span>
                    ) : null}
                </div>
            </div>
                    ...
                    <div className="row g-3">
                        ...
                        <div className="col-12">
                            ...
                            <div className="d-flex gap-2">
                                ...
                                <button type="button" className="btn btn-outline-secondary" onClick={copyLink} disabled={!shareUrl}>
                                    {copyState || 'Copy'}
                                </button>
                            </div>
                        </div>
            ...
            {!isSubmitted ? (
                <div className="card mb-3">
                    <div className="card-body d-flex flex-wrap gap-2 align-items-center">
                        <button type="button" className="btn btn-outline-primary" onClick={saveDraft} disabled={processing}>
                            Save Draft
                        </button>
...
HealthScoreShow.layout = (page) => <AuthenticatedLayout header="Health Score">{page}</AuthenticatedLayout>;
